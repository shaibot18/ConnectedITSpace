...
function newRoomStatEntry(obj) {
  ...
  const query = { _roomId: obj._roomId, recordDate: obj.recordDate };

  RoomStat.findOneAndUpdate(query, obj, { upsert: true }, (err, doc) => {
    if (err) deferred.reject(`${err.name} : ${err.message}`);
    deferred.resolve(doc);
  });
  ...
  /* RoomStat.find({
    ...
    if (res.length > 0) {
      console.log(`ROOM STAT SERVICE: record EXISTS. Lengh: ${res.length}, processing...`);

      console.log(res[0]);
      console.log(chalk.yellow(JSON.stringify(obj)));
    } else {
...
// TODO: move config to global config file
setInterval(_roomStatAggregate, 3600 * 1000);

function _roomStatAggregate() {
  console.log(chalk.red('ROOM STAT SERVICE: HOUSE KEEPING TRIGGERED...'))
  const deferred = Q.defer();
